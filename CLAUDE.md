...
**The CRM is swappable — nothing outside `app/bitrix/` imports it directly.** `app/crm/protocol.py` defines `CrmClient` (a `Protocol`): the only CRM-shaped type `app/flows/*`, `app/forms/*`, and `app/waha/*` are allowed to depend on. `BitrixClient` implements it and owns all Bitrix-specific knowledge (custom field IDs like `UF_CRM_*`, picklist value IDs, the `CONTACT_ID`/`PHONE` field shapes) behind business-vocabulary methods (`get_matricula`, `set_duplicado_status`, `get_contact_phone`, ...) — callers never see a raw Bitrix field ID. `app/crm/deps.py::get_crm_client()` branches on the `CRM_PROVIDER` env var (`app/crm/settings.py`) and is the single factory seam to swap in a different CRM later — add an `elif` there; see `app/crm/README.md`. Webhook routes themselves stay CRM-specific on purpose (see the "one endpoint per trigger" rule below) — swapping the CRM changes what's *behind* those routes, not their shape.
...
**Testing routes that use `Depends(get_x_client)`**: use `app.dependency_overrides[get_x_client] = lambda: FakeClient()` (clear it in a `finally`), not `monkeypatch.setattr(...)` — `Depends` captures the function object at route-definition time, so patching the module attribute afterward has no effect. Plain function calls that aren't wrapped in `Depends` (e.g. in `app/flows/router.py` and `app/forms/router.py`, which call `get_crm_client()`/`get_xposure_client()` directly rather than via dependency injection — deliberately, so a missing-config `RuntimeError` stays inside the caller's own try/except instead of turning into an unhandled 500 during dependency resolution) can still be monkeypatched by module path — see `tests/test_main.py` for both patterns side by side, and `tests/fakes.py::FakeCrmClient` for the shared fake used across these tests.
