...
`BitrixClient` se compone de tres mixins (límite de 500 líneas del repo,
`client.py` ya pasaba de 650): `client_deals.py` (deal, crudo y de negocio),
`client_contacts.py` (contacto, búsqueda y creación) y `client_files.py`
(timeline y Drive). `app/bitrix/_shared.py` tiene los helpers privados que
comparten (nunca se importa fuera de este paquete).
"""
...
class BitrixClient(DealsMixin, ContactsMixin, FilesMixin):
