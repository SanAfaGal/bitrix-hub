...
def get_template_editor(
    key: str, username: str = Depends(require_admin), staff_user: dict = Depends(require_staff_user)
) -> HTMLResponse | RedirectResponse:
    if key not in _template_keys():
        return RedirectResponse(url=f"{TEMPLATES_PATH}/{_template_keys()[0]}", status_code=303)
    content = templates_store.get_template(key)
    return HTMLResponse(render_template_editor_html(display_name=staff_user["name"], key=key, content=content))
...
def post_template(
    ...
    staff_user: dict = Depends(require_staff_user),
) -> HTMLResponse | RedirectResponse:
    if key not in _template_keys():
        return RedirectResponse(url=f"{TEMPLATES_PATH}/{_template_keys()[0]}", status_code=303)
    ...
    try:
        ...
        return HTMLResponse(
            render_template_editor_html(
                ...
                key=key,
                content=content,
                flash="El texto no puede estar vacío.",
                flash_error=True,
            )
        )
    ...
    return HTMLResponse(
        render_template_editor_html(display_name=staff_user["name"], key=key, content=payload.content, flash="Guardado.")
    )


@router.post(f"{TEMPLATES_PATH}/{{key}}/restore", summary="Restaura una plantilla a su valor por defecto", response_model=None)
def post_restore_template(
    key: str, username: str = Depends(require_admin), staff_user: dict = Depends(require_staff_user)
) -> HTMLResponse | RedirectResponse:
    if key not in _template_keys():
        return RedirectResponse(url=f"{TEMPLATES_PATH}/{_template_keys()[0]}", status_code=303)
    ...
    return HTMLResponse(
        render_template_editor_html(
            display_name=staff_user["name"], key=key, content=default_content, flash="Restaurado al valor por defecto."
        )
    )


@router.get(CONFIG_PATH, response_class=HTMLResponse, summary="Edita el comportamiento del bot (system prompt)")
def get_config(username: str = Depends(require_admin), staff_user: dict = Depends(require_staff_user)) -> HTMLResponse:
    content = templates_store.get_template(templates_store.CONFIG_KEY)
    return HTMLResponse(render_config_html(display_name=staff_user["name"], content=content))
...
def post_config(
    ...
    try:
        ...
        return HTMLResponse(
            render_config_html(
                display_name=staff_user["name"], content=content, flash="El texto no puede estar vacío.", flash_error=True
            )
        )
    ...
    return HTMLResponse(render_config_html(display_name=staff_user["name"], content=payload.content, flash="Guardado."))


@router.post(f"{CONFIG_PATH}/restore", summary="Restaura el comportamiento del bot al valor por defecto")
def post_restore_config(
    username: str = Depends(require_admin), staff_user: dict = Depends(require_staff_user)
) -> HTMLResponse:
    ...
    return HTMLResponse(
        render_config_html(display_name=staff_user["name"], content=default_content, flash="Restaurado al valor por defecto.")
    )


@router.get(PROSPECTS_PATH, response_class=HTMLResponse, summary="Lista los prospectos que el bot está atendiendo")
def get_prospects(username: str = Depends(require_admin), staff_user: dict = Depends(require_staff_user)) -> HTMLResponse:
    chats = conversation_store.list_chats()
    return HTMLResponse(render_prospects_html(display_name=staff_user["name"], chats=chats))
...
def get_prospect_detail(
    ...
    if selected is None or selected["channel"] == "email":
        ...
        selected_meta, selected_messages = _whatsapp_chat_detail(key)
...
def _whatsapp_chat_detail(chat_id: str) -> tuple[dict, list]:
    """Trae identidad + deal + estado del bot + historial de un chat de WhatsApp en una
    sola sesión (`ConversationStore.get_chat_detail`) en vez de las 5 llamadas sueltas que
    hacía antes esta vista — ver whatsapp_bot_store.get_chat_detail."""
    detail = conversation_store.get_chat_detail(chat_id, history_limit=100)
    if detail is None:
        return {
            "confirmed_name": None,
    ...
    meta = {
        "confirmed_name": detail["name"],
        "confirmed_phone": detail["phone"],
        "deal_id": detail["deal_id"],
        "channel": "whatsapp",
        "bot_enabled": detail["bot_enabled"],
        "bot_enabled_reason": detail["bot_enabled_reason"],
    }
    return meta, detail["messages"]


@router.get(
    f"{PROSPECTS_PATH}/{{chat_id}}/detail",
    response_class=HTMLResponse,
    summary="Fragmento AJAX del hilo de un prospecto (sin la lista) — usado por prospects.js al cambiar de chat",
)
def get_prospect_thread_fragment(chat_id: str, username: str = Depends(require_admin)) -> HTMLResponse:
    """Contraparte liviana de `get_prospect_detail`: NO llama `list_chats()` (ese scan
    completo de la tabla de mensajes es justamente lo que sobrecargaba el pool de conexiones
    con clics rápidos, ver app/admin/README.md) — solo trae y devuelve el hilo del chat
    seleccionado. `prospects.js` reemplaza `.prospect-thread-pane` con esta respuesta."""
    meta, messages = _whatsapp_chat_detail(chat_id)
    return HTMLResponse(render_thread_pane_html(chat_id=chat_id, chat_meta=meta, messages=messages))


@router.get(COVERAGE_PATH, response_class=HTMLResponse, summary="Lista sectores y su cobertura de ventas")
def get_coverage(
    estado: str = "todos", username: str = Depends(require_admin), staff_user: dict = Depends(require_staff_user)
) -> HTMLResponse:
