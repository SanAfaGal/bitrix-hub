...
`templates/shell.html` + `static/admin.css` — lo reusan las cinco vistas
(Plantillas, Configuración, Prospectos, Cobertura, WhatsApp). Cada vista
después arma su propio `body` y engancha su CSS/JS propio vía los
parámetros `extra_head_html`/`extra_body_html` de `render_app_shell` — ver
`prospects_page.py`/`coverage_page.py`/`waha_session_page.py` para el ejemplo.

Los fragmentos repetidos (badges, filas de la lista, pills del topbar,
opciones de un select, estados de la sesión de WhatsApp) son macros Jinja2
en `app/admin/jinja_templates/` (`_prospects_fragments.html`,
`_coverage_fragments.html`, `_whatsapp_fragments.html`,
...
endpoint `GET /admin/prospects/{key}/detail` en `router.py`. WhatsApp usa el
mismo patrón (`static/whatsapp.js`) para el poll de estado de la sesión y el
refresco del QR — ver el docstring de `waha_session_page.py`.
