...
def get_url() -> str:
    """Reutiliza la misma configuración de conexión que
    `app/message_templates/db.py::_build_engine()` — no se duplica ni se
    hardcodea acá."""
    s = load_message_templates_settings()
    return f"mysql+pymysql://{s.user}:{s.password}@{s.host}:{s.port}/{s.database}"


config.set_main_option("sqlalchemy.url", get_url())

# `target_metadata` combina los Base.metadata de cada paquete (cada uno con
